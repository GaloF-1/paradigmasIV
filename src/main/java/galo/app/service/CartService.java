...
public class CartService {
    ...
    public CartService(CartRepository c, ProductRepository p, OrderRepository o) {
        cartRepo=c;
        productRepo=p;
        orderRepo=o;
    }
    ...
    public Cart getOrThrow(String id){
    ...
    public Cart removeItem(String cartId, Long itemId){
        Cart cart=getOrThrow(cartId); cart.getItems().removeIf(i->i.getId().equals(itemId));
        return cartRepo.save(cart);
    }
    ...
    public Order checkout(String cartId, CheckoutRequest req){
        ...
        order.setBuyerEmail(req.email());
        order.setBuyerName(req.name());
